...
import official.nlp.bert.run_classifier as run_classifier
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if ex_index < 0:
            ...
            tf.compat.v1.logging.info("label: %s (id = %s)" % (example.labels, labels_ids))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    # model =  bert.bert_models.models.BertClassifier(
    #     config=bert_config,
    #     is_training=is_training,
    #     input_ids=input_ids,
    #     input_mask=input_mask,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if init_checkpoint:
            (assignment_map, initialized_variable_names
             ) = bert_config.get_assignment_map_from_checkpoint(tvars, init_checkpoint)
            if use_tpu:
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
                ...
                for j, logits in enumerate(logits_split):
                    ...
                    current_auc, update_op_auc = tf.compat.v1.metrics.auc(label_id_, logits)
                    eval_dict[str(j)] = (current_auc, update_op_auc)
                eval_dict['eval_loss'] = tf.compat.v1.metrics.mean(values=per_example_loss)
                return eval_dict
...
bert_config = bert.configs.BertConfig.from_json_file(BERT_CONFIG)
